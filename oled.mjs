...
class Oled {
    ...
    */
    ...
    battery = async (x, y, percentage) => {
        ...
        await this.drawPixel([[x + 17, y + 1, 1], [x + 17, y + 7, 1]]);
            ...
            fs.createReadStream(image)
                ...
                .on('parsed', async function () {
                    if (typeof animated === 'boolean' && animated) {
                        ...
                        pdyb = -1;
                        try {
                            let myInterval = setInterval(async () => { await _oled._drawPseudo(_oled, clear, this, pdxb, pdyb) }, 10);
                            timers.push(myInterval);
                        } catch (e) { console.log(e) }
                    } else {
                        await _oled.api.drawRGBAImage(this, x || Math.floor((_oled.WIDTH - this.width) / 2), y || Math.floor((_oled.HEIGHT - this.height) / 2), true);
                    }
                });
        } catch (err) {
            console.error(err);
    ...
    _drawPseudo = async (display, clear, image, pdxb, pdyb) => {
        if (typeof this._drawPseudo.init === "undefined" || this._drawPseudo.init === true || this._drawPseudo.image !== image) {
        ...
        if (clear) {
            ...
            await display.fillRect(1, 1, display.WIDTH - 2, display.HEIGHT - 2, 0, true);
        } else {
            await display.fillRect(this._drawPseudo.prevX, this._drawPseudo.prevY, image.width, image.height, 0, false);
        ...
        if (this._drawPseudo.x + this._drawPseudo.dx > display.WIDTH - image.width || this._drawPseudo.x < 1) {
            this._drawPseudo.dx = -this._drawPseudo.dx;
        }
        if (this._drawPseudo.y + this._drawPseudo.dy > display.HEIGHT - image.height || this._drawPseudo.y < 1) {
